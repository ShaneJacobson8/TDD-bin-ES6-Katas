...
describe('`Map` is a key/value map', function(){
  ...
  it('provides `new Map().set()` to add key+value pair, `get()` to read it by key', function() {
    let map = new Map();
    map.set('key', 'value');//map.set() adds params (key, value)
    const value = map.get('key');//map.get will return matching value when passed the key
  ...
  it('complex types can be keys', function() {
    ...
    //assert is looking for the map to not have a key for 'otherObj'
    //so I commented out the map.set() for that key
  });
});