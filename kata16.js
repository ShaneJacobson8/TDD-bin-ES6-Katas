...
describe('Object literal properties may be computed values', () => {
  ...
  it('can also get a function assigned', () => {
    const key = 'func';
    const obj = {[key]: () => 'seven'}; //inline function  
  ...
  it('the key can also be constructed by an expression', () => {
    const what = 'Name';
    const obj = {['property' + what]: null}; //changed key to Name, added ty to proper, resulting string becomes propertyName and is inserted as the Key
    assert('propertyName' in obj);
  });
  it('accessor keys can be computed names too', () => {
    const obj = {
      get ['key']() {return 1}, //changed set to get, removed _ from params
