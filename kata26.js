...
describe('Classes can inherit from another', () => {
  ...
  describe('`extends` using an expression', () => {
    it('e.g. the inline assignment of the parent class', () => {
      let A;
      class B extends (A = class{}) {} //reassigning variable A to be a class, on the fly
      assert.equal(new B() instanceof A, true);
    });
    it('or calling a function that returns the parent class', () => {
      const returnParent = (beNull) => beNull ? null : class {}; // ? is checking True/false, if if true, function returns first arg, else, second arg
      class B extends (returnParent(true) ){} //returnParent is a function, needs parameter so it can provide proper output
